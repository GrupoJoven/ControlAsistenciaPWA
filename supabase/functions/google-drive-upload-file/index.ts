...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!(file instanceof File)) {
      return new Response(JSON.stringify({ error: "No se recibió ningún archivo válido." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const bodyBytes = new Uint8Array(
      encoder.encode(part1).length +
      encoder.encode(part2Header).length +
      fileBuffer.length +
      encoder.encode(end).length
    );
    ...
    write(encoder.encode(part1));
    write(encoder.encode(part2Header));
    write(fileBuffer);
    write(encoder.encode(end));

    const driveRes = await fetch(
      "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&supportsAllDrives=true&fields=id,name,mimeType,modifiedTime,webViewLink,iconLink,thumbnailLink,size,owners(displayName,emailAddress),capabilities(canEdit,canRename,canTrash,canDownload,canAddChildren)",
      {
        method: "POST",
        headers: {
    ...
    if (!driveRes.ok) {
      return new Response(
        JSON.stringify({
          error: driveJson?.error?.message ?? "Google Drive rechazó la subida",
        }),
        {
