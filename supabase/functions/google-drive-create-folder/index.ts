...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const parentFolderId = folderIdRaw || rootFolderId;

    if (!name) {
      return new Response(JSON.stringify({ error: "El nombre de la carpeta no puede estar vacío" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const driveRes = await fetch(
      "https://www.googleapis.com/drive/v3/files?supportsAllDrives=true&fields=id,name,mimeType,modifiedTime,webViewLink,iconLink,thumbnailLink,size,owners(displayName,emailAddress),capabilities(canEdit,canRename,canTrash,canDownload,canAddChildren)",
      {
        method: "POST",
        headers: {
    ...
    if (!driveRes.ok) {
      return new Response(
        JSON.stringify({
          error: driveJson?.error?.message ?? "Google Drive rechazó la creación de la carpeta",
        }),
        {
